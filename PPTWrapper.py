...
def add_picture_slide(presentation, image, slide_title, subtext="", resize_or_crop = False):
    ...
    if type(image) == str:
        ...
        image_stream = BytesIO()
        image.savefig(image_stream, format="PNG", pil_kwargs = {"optimize":False, "compress_level" : 0})
...
def add_picture_with_content_slide(presentation, image, slide_title, subtext="", resize_or_crop = False):
    '''
    ...
    if type(image) == str:
        ...
        print("Warning! Not image or file name provided... Try to see if matplot figure")
        image_stream = BytesIO()
        image.savefig(image_stream, format="png", bbox_inches= 'tight')
        #image.savefig(image_stream, format="png")
        #image.savefig("c:\\temp\\opb.png", format="png")
        image_stream.seek(0)
        mimg = Image.open(image_stream)
        #mimg.show()
        #mimg.save('c:\\temp\\rawg.png', 'PNG')
